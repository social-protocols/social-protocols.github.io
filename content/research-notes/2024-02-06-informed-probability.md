...
- If we know $P(U|T=0,V=0)$ and $P(U|V=1)$ we can extrapolate $P(U|T=1,V=0)$. This is also useful if we have sparse data.
...
Another approach would be to estimate $\theta$ using the data we have for users that voted on the note. Then we can separately estimate $\theta[1]$ and $\theta[2]$, and use these to update $\theta$ using a pseudo-tally.
...
Another approach would be a linear regression. Doing a Bayesian linear regression would give us a probability distribution for the slope and intercept. From this, we can compute the probability distribution for $\theta$, $\theta[1]$, or $\theta[2]$ values.
